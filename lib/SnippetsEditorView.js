...
import ICON_MAPPINGS from './icon-mappings';

export default class SnippetsEditorView {
  constructor (uri, savedScope, savedSnippet) {
    this.uri = uri;
    ...
    this.file.read().then(str => {
      ...
      if (savedScope) this.selectScope(savedScope);
      if (savedSnippet) this.selectSnippet(savedSnippet);
    });
  }
  ...
  getIconForScope(scope) {
    return ICON_MAPPINGS[scope] || (scope.split('.').pop() + "-icon");
  }
          ...
          <ul className='panels-menu nav nav-pills nav-stacked' ref='panelMenu'>
            { Object.keys(this.snippets).map(scope =>
              <li
                ...
                onClick={() => this.selectScope(scope)}><a className={"icon icon-gist " + this.getIconForScope(scope)}>{scope}</a></li>) }
            ...
            (this.currentScope && this.currentSnippet) && <div className="edit-snippet">
              <div class="control-group">
                ...
                <div class="setting-description">The prefix is used to trigger the expansion of the
                  snippet body.</div>
                <div class="editor-container" ref="prefixEditorContainer"></div>
              </div>
              ...
              <div class="control-group">
                ...
                <div class="editor-container" ref="descriptionEditorContainer"></div>
              </div>

              <div class="control-group">
                <div class="setting-title">Body</div>
                <div class="editor-container" ref="bodyEditorContainer"></div>
              </div>
            </div>
        }
